...
def render(content, slide_num, theme, resolve_image):
    ...
    if isinstance(body, list):
        if variant == 'numbered':
            parts.append('<ol style="color:var(--color-text-secondary);font-size:20px;padding-left:28px;margin-top:14px">')
            for item in body:
            ...
            for item in body:
                parts.append(f'<li style="color:var(--color-text-secondary);font-size:20px;margin-bottom:6px">\u2022 {escape(item)}</li>')
            parts.append('</ul>')
    else:
        parts.append(f'<p style="color:var(--color-text-secondary);font-size:20px;margin-top:14px;line-height:1.5">{escape(str(body))}</p>')
