...
def render(content, slide_num, theme, resolve_image):
    ...
    if content.get('number'):
        parts.append(f'<div style="font-family:var(--font-display);font-size:60px;font-weight:700;color:var(--color-primary)">{escape(str(content["number"]))}</div>')
    ...
    if content.get('subtitle'):
        parts.append(f'<p style="font-size:18px;color:var(--color-text-muted);margin:6px 0 0 0">{escape(content["subtitle"])}</p>')
    if content.get('body'):
        parts.append(f'<p style="font-size:15px;color:var(--color-text-secondary);margin:12px auto 0 auto;max-width:70%;line-height:1.5">{escape(content["body"])}</p>')
