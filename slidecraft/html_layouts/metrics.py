...
def render(content, slide_num, theme, resolve_image):
    ...
    for metric in metrics[:4]:
        ...
        parts.append('<div>')
        parts.append(f'<div style="font-family:var(--font-display);font-size:2.4em;font-weight:700;color:var(--color-primary)">{escape(value)}</div>')
        parts.append(f'<div style="font-size:0.9em;color:var(--color-text-secondary);margin-top:8px">{escape(label)}</div>')
        if context:
            parts.append(f'<div style="font-size:0.75em;color:var(--color-text-muted);margin-top:4px">{escape(context)}</div>')
        parts.append('</div>')
    ...
    if body:
        ...
        for line in body:
            parts.append(f'<p style="color:var(--color-text-secondary);font-size:0.85em;margin:0 0 6px 0">{escape(line)}</p>')
        parts.append('</div>')
