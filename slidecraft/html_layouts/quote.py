...
def render(content, slide_num, theme, resolve_image):
    ...
    parts.append(f'<div style="font-size:72px;color:var(--color-primary);font-weight:700;line-height:1;margin-bottom:-4px">\u201C</div>')
    # Quote text
    parts.append(f'<p style="font-size:28px;color:var(--color-text);line-height:1.5;margin:0 0 14px 24px">{escape(quote_text)}</p>')
    # Attribution
    attr_text = f'\u2014 {escape(attribution)}'
    if role:
        attr_text += f', {escape(role)}'
    parts.append(f'<p style="color:var(--color-text-muted);font-size:18px;margin:0 0 0 24px">{attr_text}</p>')
