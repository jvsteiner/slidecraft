...
def _render_single(parts, profile, resolve_image):
    ...
    # Info
    parts.append('<div>')
    parts.append(f'<h3 style="font-family:var(--font-display);font-size:30px;font-weight:700;color:var(--color-text);margin:0">{escape(profile.get("name", ""))}</h3>')
    parts.append(f'<p style="color:var(--color-primary);font-weight:700;font-size:20px;margin:4px 0 0 0">{escape(profile.get("role", ""))}</p>')
    if profile.get('company'):
        parts.append(f'<p style="color:var(--color-text-muted);font-size:18px;margin:4px 0 0 0">{escape(profile["company"])}</p>')
    if profile.get('bio'):
        parts.append(f'<p style="color:var(--color-text-secondary);font-size:18px;margin:12px 0 0 0;line-height:1.5">{escape(profile["bio"])}</p>')
    parts.append('</div>')
    parts.append('</div>')
...
def _render_grid(parts, profiles, resolve_image):
    ...
    for profile in profiles:
        ...
        parts.append(f'<div style="font-family:var(--font-display);font-weight:700;font-size:20px;color:var(--color-text)">{escape(profile.get("name", ""))}</div>')
        parts.append(f'<div style="color:var(--color-primary);font-size:16px">{escape(profile.get("role", ""))}</div>')
        if profile.get('company'):
            parts.append(f'<div style="color:var(--color-text-muted);font-size:14px">{escape(profile["company"])}</div>')
        parts.append('</div>')
    parts.append('</div>')
